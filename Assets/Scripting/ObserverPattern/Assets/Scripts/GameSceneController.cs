...
public class GameSceneController : MonoBehaviour
{
    ...
    public event EnemyDestoryHandler ScoreUpdate;
    ...
    {
        foreach(IEndGameObserver obs in _endGameObservers)
        {
            obs.Notify();
        }
    }
    ...
    private IEnumerator SpawnEnemies()
    {
        ...
        for (int i = 0; i < currentLevel.numberOfEnemies; i++)
        {
            ...
            enemy.enemyDestory += Enemy_EnemyDestory;
            yield return wait;
        }
    }
    

    private void Enemy_EnemyDestory(int pointValue)
    {
        totalPoints += pointValue;

        if(ScoreUpdate != null)
        {
            ScoreUpdate(totalPoints);
        }
